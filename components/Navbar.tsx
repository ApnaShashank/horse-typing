...
import { Type, User as UserIcon, LogOut, ChevronDown, Trophy, Activity, Menu, X } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full h-16 z-60 border-b border-white/5 bg-background/50 backdrop-blur-md px-6 lg:px-8 font-mono">
        <div className="max-w-[1250px] mx-auto h-full flex items-center justify-between relative">
          ...
          <div className="flex md:hidden items-center gap-3">
            {user && (
              <Link href="/profile" className="w-8 h-8 grid-box bg-primary/10 flex items-center justify-center text-primary border border-primary/20">
                <UserIcon className="w-4 h-4" />
              </Link>
            )}
            <button 
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="grid-box p-2 hover:bg-white/5 transition-colors"
            >
              {isMobileMenuOpen ? <X className="w-4 h-4 text-on-surface" /> : <Menu className="w-4 h-4 text-on-surface" />}
            </button>
          </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-16 bg-background/95 backdrop-blur-md z-50 flex flex-col md:hidden animate-in fade-in slide-in-from-top-4 duration-200">
          <div className="flex-1 flex flex-col p-8 gap-8 font-mono">
            <div className="flex flex-col gap-5 border-b border-white/5 pb-8">
              <p className="text-[10px] uppercase font-bold text-primary/40 tracking-[0.25em]">Navigation</p>
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`text-lg font-black uppercase tracking-widest transition-colors ${pathname === link.href ? 'text-primary' : 'text-on-surface/70 hover:text-primary'}`}
                >
                  {link.label}
                </Link>
              ))}
              ...
              {user ? (
                <>
                  ...
                  <Link
                    href="/profile"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-on-surface-variant hover:text-primary transition-colors"
                  >
                ...
                <div className="flex flex-col gap-4 pt-4">
                  <Link
                    href="/login"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="w-full text-center grid-box py-4 text-xs font-bold uppercase tracking-widest text-on-surface-variant hover:text-on-surface hover:border-white/20 transition-all"
                  >
                    Sign In
                  </Link>
                  <Link
                    href="/register"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="w-full text-center grid-box py-4 text-xs font-black uppercase tracking-widest bg-primary/10 text-primary border border-primary/40 hover:bg-primary/20 transition-all"
                  >
                    Register
                  </Link>
                </div>
